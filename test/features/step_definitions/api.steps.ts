...
Given('the API is running at {string}', function (url: string) {
  baseUrl = url;
});
...
Then('the response should contain {string} as an instrument', function (ticker: string) {
  ...
  assert.strictEqual(instrument.ticker, ticker, `Expected ticker to be ${ticker}, got ${instrument.ticker}`);
...
Then('the response should contain availableCash and positions', function () {
  assert.ok(response, 'Expected a successful response');
  assert.ok('availableCash' in response, 'Expected portfolio to have availableCash');
  assert.ok('positions' in response, 'Expected portfolio to have positions');
});

When('I place a limit BUY order for {string} with size {int} and price {int}', async function (ticker: string, size: number, price: number) {
...
Then('the order should be created but marked as REJECTED', function () {
  assert.ok(response, 'Expected a successful response but got an error');
  assert.strictEqual(response.status, 'REJECTED', `Expected status to be REJECTED, got ${response.status}`);
});
